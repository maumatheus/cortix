...
// Uso: npx tsx scripts/smoke-effects.ts — gera um vídeo sintético e renderiza cada preset de efeitos.

async function main() {
  const dir = storageDir("smoke");
  const src = path.join(dir, "src.mp4");
  const gen = await run(ffmpegBin(), ["-y", "-v", "error", "-f", "lavfi", "-i", "testsrc2=size=1280x720:rate=30:duration=8", "-f", "lavfi", "-i", "sine=frequency=440:duration=8", "-c:v", "libx264", "-preset", "ultrafast", "-c:a", "aac", "-shortest", src]);
